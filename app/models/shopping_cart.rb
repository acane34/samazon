class ShoppingCart < ApplicationRecord
  ...
  scope :set_user_cart, -> (user){ user_cart = where(user_id: user.id, buy_flag: false)&.last
   ...
   scope :bought_months_pg, ->  {bought_carts.pluck("distinct(date_trunc('day', updated_at+'9 hours'))").map{|d| d.in_time_zone('Asia/Tokyo')}.reverse}
  ...
  def shipping_cost
    product_ids = ShoppingCartItem.user_cart_iten_ids(self.id)
