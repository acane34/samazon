class Product < ApplicationRecord
  ...
  scope :sort_products, -> (sort_order, page){
    on_category(sort_order[:sort_category]).sort_order(sort_order[:sort]).display_list(page)
  }
  ...
  scope :recommend_products, -> (number){where(recommended_flg: true).take(number)}
  ...
  private
    def self.csv_attributes
      [:name, :description, :price, :recommended_flg, :carriage_flag]
    end
end
